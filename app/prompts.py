...
5. בשאלות מחשבון / מכפיל / יחס המרה / נוסחה:
   פתח ב**מחשבון חיזוי** קונקרטי. אם המקורות שנשלפו כבר מציינים מכפיל, נוסחה או ספי החלטה — השתמש בהם לפני חישוב חדש מהטבלה.
   אם יש לפחות 3 זוגות כמותיים שימושיים, גזור כלל אצבע עסקי והצג נוסחה, דוגמאות, ספים ומגבלות.
   אל תחליף את הנוסחה בסירוב בגלל מדגם קטן; כתוב שזה **לא מודל סטטיסטי** ושמור את המגבלות אחרי המחשבון.
6. בשאלות על תפקיד של אלמנט בקמפיין ("היו בפרומואים X", "מה היה התפקיד של X בקמפיין") — הבחנה קריטית:
   ...
   אם השאלה או ההיסטוריה מציינות "מאסטר שף VIP" או "נבחרת החלומות", קרא לקמפיין במפורש **"מאסטר שף VIP / נבחרת החלומות, עונה 11"** גם כאשר שורות Excel מסומנות רק כ"מאסטר שף" או season="11"; אל תציג זאת כעונת מאסטר שף רגילה.
   אל תכתוב שהממצא חלקי רק בגלל שהשליפה מחזירה מדגם שורות; כתוב זאת רק אם מקור רלוונטי חסר או אם הקונטקסט מציין במפורש כיסוי חלקי.
7. בשאלות אפקטיביות ("האם זה עבד", "הוכיח את עצמו") — פתח בפסק דין: **עבד / עבד חלקית / לא עבד**.
...
# Viewing-intentions ("כוונות צפייה") appear in the source docs in TWO different
# measurement tiers that must not be conflated:
#   - promo/trailer screening test (higher, ~60-85%)
#   - pre-launch campaign / unaided field check (lower, ~35-45%)
# The section header (surfaced in Word context as "פרק: ...") identifies the tier.
_VIEWING_INTENTIONS_RE = re.compile(r"כוונות\s+ה?צפי")

_VIEWING_INTENTIONS_ADDENDUM = """

## הבחנה בין סוגי בדיקות כוונות צפייה
המסמכים מכילים שני סוגי מדידה שונים של "כוונות צפייה", ואסור לערבב ביניהם:
- **בדיקת פרומו/טריילר עם הקרנה** (מספרים גבוהים, בדרך כלל 60%-85%): זהו נתון הכותרת לשאלה כללית מסוג "מה היו כוונות הצפייה" ולהשוואת כוונות בין סדרות.
- **בדיקת כוונות לפני השקה בפועל / בלתי נעזר בקמפיין** (מספרים נמוכים, בדרך כלל 35%-45%): השתמש בו בעיקר כשמשווים כוונות מול הרייטינג/נקודת הפתיחה בפועל.

כללים:
1. לכל מספר כוונות צפייה, זהה לאיזו בדיקה הוא שייך לפי הכותרת/פרק של הקטע שנשלף ("פרק: ...").
2. לשאלה כללית "מה היו כוונות הצפייה" או להשוואה בין סדרות — הובל בנתון בדיקת הפרומו/טריילר, ואל תחליף אותו בנתון הנמוך של הבדיקה לפני השקה בפועל.
3. לשאלה על היחס בין כוונות לרייטינג בפועל — השתמש בנתון הבדיקה לפני השקה בפועל.
4. אם קיימים שני הנתונים לאותה סדרה, מותר ומומלץ לציין את שניהם ולתייג כל אחד; אך בתוך אותה השוואה שמור על עקביות — אל תשווה מספר פרומו של סדרה אחת מול מספר לפני-השקה של סדרה אחרת.
""".strip()
...
def build_messages(
    ...
    if "כיסוי שליפה רחבה" in context:
        addendum = f"{addendum}\n\n{_BROAD_RETRIEVAL_ADDENDUM}"
    if route in ("word_quote", "hybrid") and (
        _VIEWING_INTENTIONS_RE.search(user_query)
        or _VIEWING_INTENTIONS_RE.search(context)
    ):
        addendum = f"{addendum}\n\n{_VIEWING_INTENTIONS_ADDENDUM}"
