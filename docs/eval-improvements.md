...
| Jun 11, 2026 | Judge | Foundry gpt-4o | **57.0%** | **58.1%** | **ALL-TIME HIGH (+9pp judge vs Run 8).** Strategic Synthesis Mode rebuild: synthesis-phrasing triggers (סכם/תובנות/פתרונות) aligned with `_STRATEGIC_INTENT_PATTERNS`, thesis-first structure, merged insight+solution principles, Coverage-Mode precedence rule; `MAX_ANSWER_TOKENS` 1800→3000. 62 cases, 0 errors; refusal 100%, grounded 82.3% |
| Jun 11, 2026 (Gemini) | No judge | Gemini 2.5 Flash | 59.3%* | — | *Not comparable (judge off → different weights). Numeric 49.8% / grounded 91.9% beat gpt-4o; strategy 41% / cross_show 41% worse. **Blocker: thinking-block leak in 15/62 answers** (bare `thinking` prefix or unclosed `<thinking>` evades the strip regex in service.py). Fair re-judge with gpt-4o judge pending leak fix — judge shares `get_provider()` with the agent, so provider A/Bs must split generate/judge phases |
| Jun 11, 2026 (guard) | Judge | Foundry gpt-4o | 52.9%† | 57.5%† | †20-case guard (strategy/open_ended/cross_show only) validating prompt-v3 (verbatim-specifics + inline-citation rules): strategy 46→51%, open_ended 50→52%, cross_show 61→58% (N=4, within noise). No regression — v3 kept |
| Jun 11, 2026 (A/B) | Judge (gpt-4o judge, two-phase) | **Foundry gpt-5.4-mini** (reasoning_effort=low) | **62.4%†** | **67.5%†** | †Same 20-case subset — **+9.5pp overall / +10pp judge vs gpt-4o on identical prompt+retrieval, at HALF the latency (10.3s vs 21.9s avg), grounded 100%**. All 4 model-ceiling cases flipped: id=46 judge 5/5, id=56 5/5 (18.5% finally surfaced, NUM 0→0.5), id=57 0.75, id=26 0.75 (needed reasoning_effort=medium, 27.8s). Model-tier bottleneck CONFIRMED and SOLVED — gpt-5.4-mini is the production candidate. Note: judge shares the provider env — full --judge runs after switching .env will use a 5.4-mini judge (new baseline; fine for the planned ~200-question re-baseline) |
| Jun 15, 2026 (creative fine-tune) | Judge (gpt-4o judge, two-phase) | Foundry gpt-5.4-mini (effort=low) | 62.7%† | 66.2%† | †Same 20-case guard after the Creative Director Layer (human-truth-first thesis, on-screen translation, planning/retrospective register, team voice). **No regression vs the 62.4%/67.5% baseline — grounded held 100% throughout.** The thesis-altitude gap (lead with the human truth, not a media tactic) finally cracked — root cause was the Strategic Synthesis **few-shot** modeling a tactic-led opening; rewriting that one exemplar + a hard "don't open with הטעות" constraint did what 3 rounds of prose rules could not. Conclusion: the planned voice **subagent is unnecessary** — the gap closed in-prompt |
